...
static void flash_sector_erase(u32 sector_addr)
{
	if(sector_addr& (FLASH_SECTOR_SIZE-1)){		
		...
		flash_printf("[FLASH WARNING]:  flash_sector_erase addr not align\n");
	}
	u8 cmd_buf[4];
...
static void flash_block64_erase(u32 block64_addr)
{
	if(block64_addr& (FLASH_BLOCK_SIZE-1)){		
		// not align by FLASH_SECTOR_SIZE
		flash_printf("[FLASH WARNING]: flash_block64_erase addr not align\n");
	}
	u8 cmd_buf[4];
...
static void flash_page_program(const u8 *buf, u32 addr, u32 len)
{
	...
	if((len + addr&(FLASH_PAGE_SIZE-1)) > FLASH_PAGE_SIZE){		
		flash_printf("[FLASH WARNING]: write too long or not in one page\n");
	}
	u8 cmd_buf[4];
...
//  flash_printf("page_offset = %d\n",page_offset);
//	flash_printf("page_last_len = %d\n",page_last_len);
//	flash_printf("first_page_len = %d\n",first_page_len);
//	flash_printf("last_page_len = %d\n",last_page_len);
//	flash_printf("whole_page_num = %d\n",whole_page_num);
...
/* Generic command (Not used by FatFs) */
        ...
        case FLASH_CTRL_FORMAT:
            break;
/*    user cmd   */		
		case FLASH_CTRL_GET_SIZE:
			*((u32 *)buff) = flash_get_sizeKB();
			res = true;	
			break;
		case FLASH_CTRL_CHIP_ERASE:
			flash_chip_erase();
			res = true;	
			break;
		case FLASH_CTRL_GET_UNIQUE_ID:
			flash_read_unique_id(buff);
			res = true;	
			break;	
		case FLASH_CTRL_POWER_ON:
			flash_power_up();
			res = true;	
			break;	
		case FLASH_CTRL_POWER_OFF:
			flash_power_down();
			res = true;	
			break;			
		default:
...
{ 
	...
	if(flash_get_status() != DEV_ON_LINE){
		flash_printf("[WARNING]:flash DEV_OFF_LINE\n");
	}
	flash_read_jedec();
} 
...
//	u8 i = 0;
//	for(i=0;i<8;i++){
//		flash_printf("unique_id[%d]: 0x%x\n",i,unique_id[i]);
//	}
//	flash_size_kb = flash_get_sizeKB();
//	flash_printf("flash_size_kb = 0x%x\n",flash_size_kb);
//	flash_sec_cnt = (flash_get_sizeKB()<<10)/FLASH_SECTOR_SIZE;
//	flash_printf("flash_sec_cnt = %d\n",flash_sec_cnt);
//	flash_printf("HAL_GetTick = %d\n",HAL_GetTick());
//	flash_chip_erase();
//	flash_printf("HAL_GetTick = %d\n",HAL_GetTick());	
